...
public interface PizzaRepository extends JpaRepository<Pizza, Long> {
    ...
    List<Pizza> getPizzaWithPriceBetween(BigDecimal priceFrom, BigDecimal priceTo);

    @Query(value = "SELECT * FROM PIZZA WHERE IS_SPICY = :isSpicy", nativeQuery = true)
    List<Pizza> getPizzaWithIsSpicy(Boolean isSpicy);

    @Query("select p from Pizza p")
    List<Pizza> getAllSorted(Sort sort);
